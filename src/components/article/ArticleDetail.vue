      ...
      <div class="article-user">
        <div class="author-info-box">
          <a-avatar class="avatar" :src="data.avatar ? data.avatar : require('@/assets/img/default_avatar.png')"
                    :size="46" @click="routerUserCenter(data.userId)" />
          <div class="author-name-meta" style="padding-left: 10px;">
            <div class="author-name">
              <a target="_blank" class="username" @click="routerUserCenter(data.userId)">
            ...
            <span class="time">
              ...
              <span class="views-count">
               {{ $t("common.read") + ' ' + data.views }}
            </span>
            </div>
          </div>
        </div>
        <div class="follow-box">
          <div class="edit" v-if="$store.state.userId === data.userId">
...
<!--            </a-button>-->
      ...
      <div class="article-titleMap" v-if="data.postImg">
        <img :src="data.postImg" style="width: 100%;" />
      </div>
      <div class="article-content" style="width: 100%" v-if="data.postContent">
        <mavon-editor
          v-model="data.postContent"
          :subfield="false"
          defaultOpen="preview"
          :toolbarsFlag="false"
          boxShadowStyle="0"
...
export default {
  ...
  methods: {
    ...
    getArticleById() {
      this.finish = false;
      postService.detail({ postId: this.$route.params.id, isPv: true })
        .then(res => {
          this.data = res.data;
          this.finish = true;
          // 提取标签id
          let labelIds = [];
          res.data.categoryVOS.forEach(category => {
            labelIds.push(category.categoryId);
          });
          this.$emit("initLabelIds", labelIds, this.finish, res.data.userId, this.$utils.toToc(res.data.postContent));
